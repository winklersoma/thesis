...
time_step = 0.001
Data = []
for i in range(20):  # The simulation will run 100 times!
...
plt.title('Exponential decay of ' + str(D0) + ' drug molecules at a decay rate of ' + str(r))
...
plt.plot(stat_time, mean + np.sqrt(var), linewidth=1, color="black", label='Mean level +/- standard deviation')
plt.plot(stat_time, mean - np.sqrt(var), linewidth=1, color="black")
# plt.plot(DT, DE, linewidth=1.2, color="red", label='Expected drug level in model')
plt.xlabel("Time")
plt.ylabel("Number of drug molecules")
plt.legend()
plt.show()
